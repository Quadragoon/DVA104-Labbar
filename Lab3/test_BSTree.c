...
void testTree(BSTree tree)
{
    ...
    assert(depth(tree) == minDepth(tree) && minDepth(tree) == 0);
	...
	assert(depth(tree) == 3); // ACTUAL: om roten ‰r 0 blir djupet 3
    //assert(minDepth(tree) == 3); // Med 7 noder ar minimidjupet 3
	assert(minDepth(tree) == 2); // ACTUAL: med 7 noder ‰r minimidjupet 2
